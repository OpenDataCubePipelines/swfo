...
class BrdfShape(unittest.TestCase):
    ...
    def test_get_rms_indices(self):
        ...
        rms_val = np.sqrt(
            (0.015683596 * alpha1 ** 2)
            + (2 * 0.055165295 * alpha1 * alpha2)
            + (0.371423479 * alpha2 ** 2)
        )
    ...
    def test_mean_shape_param(self):
        ...
        alpha1, alpha2 = brdf_shape.get_mean_shape_param(
            np.mean(fiso, axis=0), np.mean(fvol, axis=0), np.mean(fgeo, axis=0), cov_iso
        )
    ...
    def test_cov_shape_param(self):
        ...
        alpha1_cov, alpha2_cov = brdf_shape.get_cov_shape_param(0.0, 0.0, 0.0)
        self.assertEqual(alpha2_cov, 0.0)
        self.assertEqual(alpha1_cov, 0.0)
        alpha1_cov, alpha2_cov = brdf_shape.get_cov_shape_param(1.0, 1.0, 1.0)
    ...
    def test_std_rms_indices(self):
        ...
        rms_val = np.sqrt(
            (0.015683596 + 0.055165295) ** 2 + (0.055165295 + 0.371423479) ** 2
        )
