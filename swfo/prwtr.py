...
def convert_file(
    fname, out_h5: h5py.Group, compression, filter_opts: Optional[Dict] = None
):
    """
...
def generate_fallback_metadata(
    ...
    md["id"] = str(
        uuid.uuid5(
            ...
            PRODUCT_HREF
            + "&"
            + urllib.parse.urlencode({"noaa_c_c_prwtreatm_1": lineage_ids}),
        )
    )
    md["product"] = {"href": PRODUCT_HREF}
    md["crs"] = src_md["crs"]
    md["datetime"] = obs_dt.isoformat()
    md["geometry"] = src_md["geometry"]
    md["grids"] = src_md["grids"]
    md["lineage"] = {"noaa_c_c_prwtreatm_1": lineage_ids}
    md["measurements"] = {"water_vapour": {"path": "", "layer": dataset_path}}
    md["properties"] = {
        "dtr:start_datetime": start_dt.isoformat(),
        "dtr:end_datetime": end_dt.isoformat(),
        "item:providers": {
            "name": "Geoscience Australia",
            "roles": [ItemProvider.PRODUCER.value, ItemProvider.HOST.value],
        },
    ...
    for _, row in group_df.iterrows():
        with h5py.File(row[0]) as fid:
            doc = h5utils.YAML.load(
                fid[
                    "{}/{}/{}".format(
                        h5utils.PUBLIC_NAMESPACE, row[2], h5utils.METADATA_PTR
                    )
                ][()].item()
            )
            uuids.append(doc["id"])
...
def fallback(
    ...
    with h5utils.atomic_h5_write(out_fname, "w", track_order=True) as fid:
        ...
        for band_index, item in enumerate(groups):
            ...
            # dataset description
            description = (
                "Average data for {year_month} {hour} hours, "
