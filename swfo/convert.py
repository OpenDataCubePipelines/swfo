...
from .h5utils import write_h5_md, atomic_h5_write
...
def _compression_options(f):
    ...
    f = click.option(
        "--filter-opts", type=JsonType(), envvar="SWFO_H5_FILTER_OPTS", default=None
    )(f)
    return f
...
def jaxa_dsm_cli():
    """
    JAXA DSM conversion preliminary library
    """
...
def mcd43a1_tiles(indir, outdir, compression, filter_opts):
    ...
    for fname in indir.rglob("*.hdf"):
        ...
        with atomic_h5_write(out_fname, "w") as out_h5:
            mcd43a1.convert_tile(str(fname), out_h5, compression, filter_opts)


@mcd43a1_cli.command(
    "h5-md", help="Convert MCD43A1 hdf4 tile to HDF5 tile with ODC metadata file."
)
@click.option(
    "--fname",
...
def mcd43a1_tile_with_md(fname, outdir, md_file, compression, filter_opts):
    ...
    for band in md["measurements"]:
        ...
        if "band" in md["measurements"][band]:
            md["measurements"][band]["band"] = mcd43a1.OUT_DTYPE.names[
                int(md["measurements"][band]["band"]) - 1
            ]

    md["properties"]["item:providers"].append(
        {"name": "GeoscienceAustralia", "roles": [ItemProvider.HOST.value]}
    )

    with atomic_h5_write(out_fname, "w") as out_h5:
        mcd43a1.convert_tile(str(infile), out_h5, compression, filter_opts)
        write_h5_md(out_h5, [md], ["/"])


@mcd43a1_cli.command(
    "vrt", help="Build VRT mosaic of each for each MCD43A1 HDF4 subdataset."
)
@_io_dir_options
...
def mcd43a1_h5(indir, outdir, compression, filter_opts):
    ...
    for day in indir.iterdir():
        ...
        attrs = {"description": "MCD43A1 product, mosaiced and converted to HDF5."}
...
def pr_wtr_cmd(indir, outdir, compression, filter_opts):
    ...
    for fname in indir.rglob("pr_wtr*.nc"):
        ...
        out_fname.parent.mkdir(parents=True, exist_ok=True)
        with atomic_h5_write(out_fname, "w") as out_h5:
            prwtr.convert_file(str(fname), out_h5, compression, filter_opts)


@prwtr_cli.command(
    "h5-md", help="Convert PR_WTR NetCDF files into HDF5 files with metadata entries"
)
@click.option(
    "--fname",
    type=click.Path(dir_okay=False, file_okay=True),
...
@click.option(
    "--out-dataset-path",
    default="/SRTM/GA-DSM",
    help="HDF5 dataset path to output dataset",
)
@_compression_options
def ga_dsm(fname, out_fname, out_dataset_path, compression, filter_opts):
    """
    ...
    attrs = {
        "description": (
            "1 second DSM derived from the SRTM; " "Shuttle Radar Topography Mission"
        )
    }
    with atomic_h5_write(out_fname, "w") as out_h5:
...
@_compression_options
def ga_dsm_md(fname, out_fname, out_dataset_path, compression, filter_opts):
    """
...
@click.option(
    "--out-dataset-prefix",
    default="/JAXA-ALOS",
    help="HDF5 dataset path to output datasets",
)
@_compression_options
def jaxa_h5(indir, out_fname, out_dataset_prefix, compression, filter_opts):
    """
    ...
    with atomic_h5_write(out_fname, "a") as out_h5:
        for vrt_fname in indir.rglob("*.vrt"):
            dsname = Path(out_dataset_prefix).joinpath(vrt_fname.stem)
            dsm.convert_file(str(vrt_fname), out_h5, dsname, compression, filter_opts)


@jaxa_dsm_cli.command("h5", help="Convert a JAXA DSM .tar.gz file into a HDF5 file.")
@_io_dir_options
@click.option(
    "--out-dataset-prefix", default="/", help="HDF5 dataset path to output datasets"
)
@_compression_options
def jaxa_tiles(indir, outdir, out_dataset_prefix, compression, filter_opts):
    """
    ...
    for fname in indir.rglob("*.tar.gz"):
        ...
        with atomic_h5_write(out_fname, "a") as out_h5:
            dsm.jaxa_tile(fname, out_h5, out_dataset_prefix, compression, filter_opts)


@jaxa_dsm_cli.command(
    "h5-md", help="Convert a JAXA DSM .tar.gz file into a HDF5 file with metadata."
)
@_io_dir_options
@click.option(
    "--out-dataset-prefix", default="/", help="HDF5 dataset path to output datasets"
...
@_compression_options
def jaxa_tiles_md(indir, outdir, out_dataset_prefix, compression, filter_opts):
    """
    ...
    for fname in indir.rglob("*.tar.gz"):
        ...
        with atomic_h5_write(out_fname, "a") as out_h5:
            md, dataset_names = dsm.jaxa_tile(
                fname, out_h5, out_dataset_prefix, compression, filter_opts
            )
            write_h5_md(out_h5, md, dataset_names)
...
def atsr2_files_md(indir, out_fname, compression, filter_opts):
    ...
    with atomic_h5_write(out_fname, "w") as out_h5:
        md, dataset_names = atsr2_aot.convert(indir, out_h5, compression, filter_opts)
        write_h5_md(out_h5, md, dataset_names)
...
@ozone_cli.command(
    "h5-md", help="Converts ozone .tif directory to a HDF5 collection with metadata."
)
@click.option(
    "--indir",
    type=click.Path(dir_okay=True, file_okay=False),
...
def ozone_files_md(indir, out_fname, compression, filter_opts):
    ...
    with atomic_h5_write(out_fname, "w", track_order=True) as out_h5:
        md, dataset_names = ozone.convert(indir, out_h5, compression, filter_opts)
        write_h5_md(out_h5, md, dataset_names)
